...
class BasePhaseTrainer:
    ...
    def _strict_geometry_energy_kwargs(self) -> Dict[str, Any]:
        """Return the single GeometryBank energy contract used by the stack.

        The classifier, replay filter, base margin, replay statistics, and
        incremental losses must use identical energy.  Do not inherit historical
        defaults here: older trainer files defaulted log-determinant energy to
        ``True`` and residual scaling to ``0.75``, which the current loss correctly
        rejects.
        """
        residual_scale = self._base_cfg_float("residual_variance_scale", 1.0)
        normalize_by_dim = self._base_cfg_bool("energy_normalize_by_dim", True)
        use_logdet = self._base_cfg_bool("use_logdet_energy", False)
        logdet_weight = self._base_cfg_float("logdet_energy_weight", 0.0)
        center_logdet = self._base_cfg_bool("center_logdet_energy", False)
        reliability_weight = self._base_cfg_float("reliability_energy_weight", 0.0)
        use_spectral_energy = self._base_cfg_bool("use_spectral_residual_energy", False)
        spectral_weight = self._base_cfg_float("spectral_energy_weight", 0.0)

        failures: List[str] = []
        if abs(residual_scale - 1.0) > 1e-8:
            failures.append(f"residual_variance_scale={residual_scale} (required 1.0)")
        if not normalize_by_dim:
            failures.append("energy_normalize_by_dim=false (required true)")
        if use_logdet or abs(logdet_weight) > 1e-12 or center_logdet:
            failures.append(
                "log-determinant energy enabled "
                f"(use={use_logdet}, weight={logdet_weight}, center={center_logdet})"
            )
        if abs(reliability_weight) > 1e-12:
            failures.append(
                f"reliability_energy_weight={reliability_weight} (required 0.0)"
            )
        if use_spectral_energy or abs(spectral_weight) > 1e-12:
            failures.append(
                "spectral classifier energy enabled "
                f"(use={use_spectral_energy}, weight={spectral_weight})"
            )
        if failures:
            raise RuntimeError(
                "Base trainer energy contract disagrees with GeometryBank replay energy: "
                + "; ".join(failures)
            )

        return {
            "variance_floor": self._base_cfg_float("geom_var_floor", 5e-4),
            "residual_variance_scale": 1.0,
            "normalize_by_dim": True,
            "invalid_class_energy": self._base_cfg_float("invalid_class_energy", 1e6),
            "use_logdet_energy": False,
            "logdet_energy_weight": 0.0,
            "center_logdet_energy": False,
            "reliability_energy_weight": 0.0,
            "use_spectral_residual_energy": False,
    ...
    def _enforce_base_contract(self) -> None:
        ...
        and builds a clean Spectral-Coupled Tangent Geometry Replay / GeometryBank handoff.
        """
        ...
        if self._base_cfg_bool("use_mssl_loss", False) and self._base_cfg_float("mssl_weight", 0.0) > 0.0:
            if not self._base_cfg_bool("unsafe_ablation_use_mssl_loss", False):
                raise RuntimeError(
                    "base trainer uses one coherent objective: CE + SRPGR. "
        ...
        # must judge feature GeometryBank quality only; no spectral scoring
    ...
    def _assert_mandatory_base_objective_config(self) -> None:
        ...
        required_positive = {
            ...
            "base_energy_margin_weight": self._base_cfg_float("base_energy_margin_weight", 0.15),
            "base_energy_margin": self._base_cfg_float("base_energy_margin", 0.25),
        ...
        if not self._base_cfg_bool("base_class_balance", True):
            raise RuntimeError("base_class_balance must be true in the main base phase.")
    ...
    def _assert_mandatory_base_batch_inputs(
        ...
        *,
        ...
        # A highly imbalanced loader may occasionally emit a single-class batch.
        # Do not crash the run; pairwise terms become zero for that batch and
        # epoch-level coverage checks still require the objective to be exercised.
    ...
    def _assert_mandatory_base_loss_parts(self, loss_out: Dict[str, Any], *, spectral_summary_is_physical: bool) -> None:
        ...
        required = (
            ...
            "base_pgr_band_guided_conflict_mean", "base_pgr_band_guided_conflict_max",
            "base_energy_margin", "base_energy_margin_weighted",
            "base_energy_margin_violation", "base_energy_margin_gap",
            "base_spectral_shape_raw", "base_spectral_shape_pair_count",
            "base_spectral_shape_guided_conflict_mean", "base_spectral_shape_guided_conflict_max",
        )
        ...
        # Structural coverage is enforced over the whole epoch, not on every
        # mini-batch.  This keeps the method valid for tiny and imbalanced HSI
        # classes without silently dropping the regularizers globally.
    ...
    def _force_base_validation_geometry_only(self) -> Dict[str, Any]:
        ...
        certificate based on spectral scoring can hide a dirty feature
    ...
    def _class_balanced_mean(values: torch.Tensor, labels_local: torch.Tensor) -> torch.Tensor:
        """Average per-class means, not per-sample means.

        Replay counts and HSI class frequencies must change coverage, not class
        weight.  This exact class average is therefore used by both CE and the
        base geometry-margin objective.
        """
        if values.dim() != 1 or labels_local.dim() != 1 or values.numel() != labels_local.numel():
            raise RuntimeError(
                f"class-balanced mean expects aligned vectors, got values={tuple(values.shape)}, "
                f"labels={tuple(labels_local.shape)}"
            )
        class_means = []
        ...
        return torch.stack(class_means).mean() if class_means else values.sum() * 0.0
    ...
    def _batch_balanced_ce(self, logits: torch.Tensor, labels_local: torch.Tensor, num_classes: int) -> torch.Tensor:
        ...
        if logits.dim() != 2 or logits.size(1) != int(num_classes):
            raise RuntimeError(
                f"base CE logits must be [B,{int(num_classes)}], got {tuple(logits.shape)}"
            )
        per_sample = F.cross_entropy(
            logits, labels_local, label_smoothing=label_smoothing, reduction="none"
        )
        return self._class_balanced_mean(per_sample, labels_local)

    def _base_batch_geometry_energy_margin(
        self,
        features: torch.Tensor,
        labels_local: torch.Tensor,
        *,
        ...
        """Differentiable base geometry margin in the canonical feature space.

        The active loss module does not own this term.  Building it here avoids
        silently logging a non-existent margin.  Class tangent bases and scales
        are estimated from detached batch statistics for numerical stability;
        gradients shape the sample positions and class centers only.
        """
        z0 = self._zero(features)
        result = {
            "base_energy_margin": z0,
            "base_energy_margin_weighted": z0,
            "base_energy_margin_violation": z0.detach(),
            "base_energy_margin_gap": z0.detach(),
            "base_energy_margin_min_gap": z0.detach(),
            "base_energy_margin_class_count": z0.detach(),
        }
        if geometry_energy_matrix is None or features.dim() != 2 or features.size(0) == 0:
            return result

        present = torch.unique(labels_local, sorted=True)
        if present.numel() < 2:
            return result
        D = int(features.size(1))
        rank_cap = min(
            max(0, self._base_cfg_int("base_energy_batch_rank", self._base_cfg_int("pgr_subspace_rank", 3))),
            D,
        )
        variance_floor = self._base_cfg_float("geom_var_floor", 5e-4)
        energy_threshold = self._base_cfg_float("rank_energy_threshold", 0.90)
        eigen_ratio_threshold = self._base_cfg_float("rank_eigen_ratio_threshold", 1e-2)
        min_rank = max(0, self._base_cfg_int("min_active_rank", 1))

        means: List[torch.Tensor] = []
        bases: List[torch.Tensor] = []
        eigvals: List[torch.Tensor] = []
        resvars: List[torch.Tensor] = []
        ranks: List[int] = []
        counts: List[float] = []
        local_target = torch.full_like(labels_local, -1)

        for bank_col, cls_t in enumerate(present):
            mask = labels_local == cls_t
            zc = features[mask]
            n = int(zc.size(0))
            mu = zc.mean(dim=0)
            means.append(mu)
            local_target[mask] = int(bank_col)
            counts.append(float(n))

            basis_pad = torch.zeros((D, rank_cap), device=features.device, dtype=features.dtype)
            eig_pad = torch.full((rank_cap,), variance_floor, device=features.device, dtype=features.dtype)
            active_rank = 0
            with torch.no_grad():
                centered = zc.detach() - mu.detach().unsqueeze(0)
                if n >= 2 and rank_cap > 0 and float(centered.abs().sum().item()) > 0.0:
                    try:
                        _, singular, vh = torch.linalg.svd(centered, full_matrices=False)
                        usable = min(rank_cap, max(n - 1, 0), int(vh.size(0)))
                        if usable > 0:
                            spectrum = singular[:usable].pow(2) / float(max(n - 1, 1))
                            total = singular.pow(2).sum().clamp_min(1e-12)
                            cumulative = singular[:usable].pow(2).cumsum(0) / total
                            energy_rank = int(
                                torch.searchsorted(
                                    cumulative,
                                    torch.tensor(energy_threshold, device=cumulative.device, dtype=cumulative.dtype),
                                ).item()
                            ) + 1
                            ratio_rank = int((spectrum / spectrum[0].clamp_min(1e-12) >= eigen_ratio_threshold).sum().item())
                            active_rank = min(usable, max(min_rank, min(energy_rank, max(ratio_rank, 1))))
                            basis_pad[:, :active_rank] = vh[:active_rank].transpose(0, 1)
                            eig_pad[:active_rank] = spectrum[:active_rank].clamp_min(variance_floor)
                    except RuntimeError:
                        active_rank = 0

                if active_rank > 0:
                    coeff = centered @ basis_pad[:, :active_rank]
                    recon = coeff @ basis_pad[:, :active_rank].transpose(0, 1)
                    residual = centered - recon
                else:
                    residual = centered
                residual_var = residual.pow(2).sum(dim=1).mean() / float(max(D - active_rank, 1))
                residual_var = residual_var.clamp_min(variance_floor)

            bases.append(basis_pad)
            eigvals.append(eig_pad)
            resvars.append(residual_var)
            ranks.append(active_rank)

        means_t = torch.stack(means, dim=0)
        bases_t = torch.stack(bases, dim=0).detach()
        eig_t = torch.stack(eigvals, dim=0).detach()
        res_t = torch.stack(resvars, dim=0).detach()
        ranks_t = torch.as_tensor(ranks, device=features.device, dtype=torch.long)
        counts_t = torch.as_tensor(counts, device=features.device, dtype=features.dtype)
        ...
        energy = geometry_energy_matrix(
            ...
            eigvals=eig_t,
            res_vars=res_t,
            active_ranks=ranks_t,
            sample_counts=counts_t,
            reliability=None,
            **{
                **self._strict_geometry_energy_kwargs(),
                "variance_floor": variance_floor,
            },
            return_parts=False,
        )
        true_e = energy.gather(1, local_target.view(-1, 1)).squeeze(1)
        true_mask = torch.zeros_like(energy, dtype=torch.bool).scatter(1, local_target.view(-1, 1), True)
        rival_e = energy.masked_fill(true_mask, float("inf")).min(dim=1).values
        gap = rival_e - true_e
        margin_value = self._base_cfg_float("base_energy_margin", 0.25)
        per_sample_loss = F.relu(float(margin_value) - gap)
        raw = self._class_balanced_mean(per_sample_loss, labels_local)
        weighted = self._base_cfg_float("base_energy_margin_weight", 0.15) * raw
        violation = self._class_balanced_mean((gap <= 0.0).to(features.dtype), labels_local)
        mean_gap = self._class_balanced_mean(gap, labels_local)

        return {
            "base_energy_margin": raw,
            "base_energy_margin_weighted": weighted,
            "base_energy_margin_violation": violation.detach(),
            "base_energy_margin_gap": mean_gap.detach(),
            "base_energy_margin_min_gap": gap.min().detach(),
            "base_energy_margin_class_count": torch.tensor(float(present.numel()), device=features.device, dtype=features.dtype),
        }
    ...
    def _base_spectral_shape_regularizer(
        ...
        *,
        ...
        SCTGR loss file exposes spectral_shape_discrimination_loss.
        """
    ...
    @torch.no_grad()
    def _update_base_replay_statistics(
        self,
        loader,
        phase_class_ids: Iterable[int],
        *,
        ...
        """Populate aggregate true-energy and margin quantiles for replay.

        GeometryBank row construction is class-local and therefore cannot know
        rival energies.  This pass measures the complete base classifier field
        and stores only aggregate quantiles; no old feature or spectrum is kept.
        """
        classes = [int(c) for c in phase_class_ids]
        if len(classes) < 2:
            raise RuntimeError("At least two base classes are required to compute replay margin statistics.")
        gb = getattr(self.model, "geometry_bank", None)
        if gb is None or not hasattr(gb, "update_energy_margin_statistics"):
            raise AttributeError("GeometryBank.update_energy_margin_statistics() is required.")
        bank = self._get_valid_base_bank(max(classes))
        idx = torch.as_tensor(classes, device=self.device, dtype=torch.long)
        energies: Dict[int, List[torch.Tensor]] = {c: [] for c in classes}
        margins: Dict[int, List[torch.Tensor]] = {c: [] for c in classes}
        was_training = bool(self.model.training)
        self.model.eval()
        try:
            for batch in loader:
                ...
                ext_s = self._normalize_external_spectra(spectra, x)
                if ext_s is not None:
                    pca_components = int(getattr(self.args, "pca_components", 0) or 0)
                    uses_reduction = pca_components > 0 and not bool(getattr(self.args, "no_pca", False))
                    reduced_metadata = bool(uses_reduction and int(ext_s.size(1)) == int(x.size(1)))
                    physical = bool(self._base_cfg_bool("external_spectra_are_physical", True) and not reduced_metadata)
                    input_s = ext_s
                else:
                    input_s = self._center_spectrum_from_input(x)
                    physical = self._base_spectral_summary_is_physical(None)
                out = self._call_extract_projected_features(
                    x, spectral_summary=input_s, spectral_summary_is_physical=physical
                )
                z = self._assert_feature_space_consistency(out)
                if hasattr(self, "_dual_geometry_energy_matrix"):
                    energy_full = self._dual_geometry_energy_matrix(z, bank, return_parts=False)
                else:
                    energy_full = geometry_energy_matrix(
                        features=z, bank=bank,
                        **self._strict_geometry_energy_kwargs(),
                    )
                energy = energy_full.index_select(1, idx)
                local = torch.full_like(y, -1)
                for pos, cls in enumerate(classes):
                    local[y == int(cls)] = int(pos)
                valid = local >= 0
                ...
                lv = local[valid]
                yv = y[valid]
                true = e.gather(1, lv.view(-1, 1)).squeeze(1)
                mask = torch.zeros_like(e, dtype=torch.bool).scatter(1, lv.view(-1, 1), True)
                gap = e.masked_fill(mask, float("inf")).min(dim=1).values - true
                for cls in classes:
                    m = yv == int(cls)
                    if bool(m.any().item()):
                        energies[cls].append(true[m].detach().cpu())
                        margins[cls].append(gap[m].detach().cpu())
        finally:
            self.model.train(was_training)

        report: Dict[int, Dict[str, float]] = {}
        for cls in classes:
            if not energies[cls] or not margins[cls]:
                raise RuntimeError(f"No energy/margin observations collected for base class {cls}.")
            e = torch.cat(energies[cls], dim=0).to(self.device)
            m = torch.cat(margins[cls], dim=0).to(self.device)
            gb.update_energy_margin_statistics(
                cls, true_energies=e, margins=m, allow_frozen_update=bool(allow_frozen_update)
            )
            report[cls] = {
                "n": float(e.numel()),
                "energy_q50": float(torch.quantile(e, 0.50).item()),
                "energy_q95": float(torch.quantile(e, 0.95).item()),
                "margin_q05": float(torch.quantile(m, 0.05).item()),
                "margin_q10": float(torch.quantile(m, 0.10).item()),
                "margin_min": float(m.min().item()),
                "violation_rate": float((m <= 0.0).float().mean().item()),
            }
        return report
    ...
    def _build_base_geometry_certificate(
        ...
        *,
        ...
        """Create the base-to-incremental SCTGR geometry certificate.

        Important semantic fix:
        raw band/spectral similarity is **not** a hard failure by itself.
        In HSI, two classes can be physically similar.  The base handoff is unsafe
        only when high physical similarity is accompanied by unsafe learned
        feature geometry.  Therefore the hard checks are geometry-energy and
        band/spectral-guided geometry conflict, not raw spectral similarity.
        """
        classes = [int(c) for c in phase_class_ids]
        cert: Dict[str, Any] = {
            "phase": 0,
        ...
        rel_v = _take(rel, 0.0)
        spectral_rank_v = _take(bank.get("spectral_active_ranks"), 0.0)
        coupling_rel_v = _take(bank.get("coupling_reliability"), 0.0)
        energy_q = bank.get("energy_quantiles", None)
        margin_q = bank.get("margin_quantiles", None)
        if torch.is_tensor(energy_q) and energy_q.dim() == 2 and energy_q.size(0) > int(idx.max().item()):
            energy_q_v = energy_q.to(self.device).index_select(0, idx).float()
        else:
            energy_q_v = torch.zeros((len(classes), 4), device=self.device)
        if torch.is_tensor(margin_q) and margin_q.dim() == 2 and margin_q.size(0) > int(idx.max().item()):
            margin_q_v = margin_q.to(self.device).index_select(0, idx).float()
        else:
            margin_q_v = torch.zeros((len(classes), 2), device=self.device)
        ...
        center_mean, center_min = 0.0, 0.0
        ...
        thr = {
            ...
            # Strict subspace overlap is diagnostic only.  The handoff fails only
            # when overlap exceeds the warning cap or when energy/guided conflict fails.
            "max_subspace_overlap": self._base_cfg_float("base_cert_max_subspace_overlap", 0.55),
            "subspace_warn_overlap": self._base_cfg_float("base_cert_subspace_warn_overlap", 0.72),
            ...
            "max_guided_geometry_conflict": self._base_cfg_float("base_cert_max_guided_geometry_conflict", 0.18),
        }

        def _guided_conflict(raw_sim: torch.Tensor, raw_thr: float) -> torch.Tensor:
            """Hard physical-pair × unsafe learned-geometry pressure."""
            if raw_sim.numel() == 0:
                return torch.zeros_like(raw_sim)
            raw_den = max(1.0 - float(raw_thr), 1e-6)
            hard_pair = F.relu(raw_sim.float() - float(raw_thr)) / raw_den

            sub_den = max(1.0 - float(thr["subspace_warn_overlap"]), 1e-6)
            sub_pressure = F.relu(sub.float() - float(thr["subspace_warn_overlap"])) / sub_den

            conf_den = max(float(thr["max_geometry_conflict"]), 1e-6)
            conflict_pressure = F.relu(conflict.float() - float(thr["max_geometry_conflict"])) / conf_den

            feature_pressure = torch.maximum(sub_pressure, conflict_pressure).clamp_min(0.0)
            guided = (hard_pair * feature_pressure).masked_fill(eye, 0.0)
            return torch.nan_to_num(guided, nan=0.0, posinf=1e3, neginf=0.0)

        band_guided = _guided_conflict(band, thr["max_band_similarity"])
        spec_guided = _guided_conflict(spec, thr["max_spectral_shape_similarity"])
        band_guided_mean, band_guided_max, _ = _pair_mean_max(band_guided)
        spec_guided_mean, spec_guided_max, _ = _pair_mean_max(spec_guided)
        ...
        cert.update({
            ...
            "spectral_active_rank": {int(c): int(spectral_rank_v[i].detach().cpu().item()) for i, c in enumerate(classes)},
            "coupling_reliability": {int(c): float(coupling_rel_v[i].detach().cpu().item()) for i, c in enumerate(classes)},
            "min_coupling_reliability": float(coupling_rel_v.min().detach().cpu().item()) if coupling_rel_v.numel() else 0.0,
            "mean_coupling_reliability": float(coupling_rel_v.mean().detach().cpu().item()) if coupling_rel_v.numel() else 0.0,
            "energy_quantiles": {int(c): energy_q_v[i].detach().cpu().tolist() for i, c in enumerate(classes)},
            "margin_quantiles": {int(c): margin_q_v[i].detach().cpu().tolist() for i, c in enumerate(classes)},
            ...
            "mean_band_guided_geometry_conflict": band_guided_mean,
            "max_band_guided_geometry_conflict": band_guided_max,
            "mean_spectral_guided_geometry_conflict": spec_guided_mean,
            "max_spectral_guided_geometry_conflict": spec_guided_max,
            "geometry_reserve_score": float(geom.get("geometry_reserve_score", max(0.0, 1.0 - conflict_max - max(band_guided_max, spec_guided_max)))),
            ...
            "band_guided_geometry_conflict_matrix": band_guided.detach().cpu().tolist(),
            "spectral_guided_geometry_conflict_matrix": spec_guided.detach().cpu().tolist(),
            "thresholds": thr,
        })

        # Warnings: raw physical similarity and strict subspace overlap are
        # informative hard-pair diagnostics, not automatic failures.
        if sub_max > thr["max_subspace_overlap"]:
            cert["warnings"].append("strict_subspace_overlap_high_but_energy_guided_gate_used")
        if band_max > thr["max_band_similarity"]:
            cert["warnings"].append("raw_band_similarity_high_used_as_hard_pair_weight")
        if spec_pair_count > 0 and spec_max > thr["max_spectral_shape_similarity"]:
            cert["warnings"].append("raw_spectral_shape_similarity_high_used_as_hard_pair_weight")

        self._compute_pair_risks(cert, center, sub, conflict, band, spec)
        self._add_handoff_recommendations(cert)

        guided_max = max(
            float(cert.get("max_band_guided_geometry_conflict", 0.0)),
            float(cert.get("max_spectral_guided_geometry_conflict", 0.0)),
        )
        # Geometry conflict is a bank-level risk score, not an energy accuracy.
        # The strict threshold is kept as the preferred operating point, but a
        # small soft band is allowed when the actual guided-geometry conflict is
        # clean and the base geometry classifier is already highly accurate.
        # This prevents meaningless FAILs such as 1.353 vs 1.350 while still
        # rejecting truly unsafe geometry.
        conflict_soft_cap = self._base_cfg_float(
            "base_cert_max_geometry_conflict_soft",
            max(float(thr["max_geometry_conflict"]) + 0.05, float(thr["max_geometry_conflict"]) * 1.05),
        )
        energy_geometry_preferred_ok = cert["max_geometry_conflict"] <= thr["max_geometry_conflict"]
        energy_geometry_soft_ok = (
            cert["max_geometry_conflict"] <= conflict_soft_cap
            and guided_max <= thr["max_guided_geometry_conflict"]
            and cert["geom_val_acc"] >= thr["min_geom_acc"]
        )
        if (not energy_geometry_preferred_ok) and energy_geometry_soft_ok:
            cert["warnings"].append(
                "geometry_conflict_slightly_above_strict_threshold_but_guided_energy_safe"
            )
        cert["max_guided_geometry_conflict"] = guided_max
        cert["energy_geometry_preferred_ok"] = bool(energy_geometry_preferred_ok)
        cert["energy_geometry_soft_ok"] = bool(energy_geometry_soft_ok)

        require_coupling = self._base_cfg_bool(
            "base_require_spectral_coupling",
            self._base_cfg_bool("use_spectral_coupled_replay", True),
        )
        coupling_ready = bool(
            (spectral_rank_v > 0).all().item()
            and torch.isfinite(coupling_rel_v).all().item()
            and (coupling_rel_v >= self._base_cfg_float("coupling_min_reliability", 0.20)).all().item()
        )
        replay_stats_ready = bool(
            torch.isfinite(energy_q_v).all().item()
            and torch.isfinite(margin_q_v).all().item()
            and (energy_q_v[:, 1:] >= energy_q_v[:, :-1] - 1e-7).all().item()
            and (margin_q_v[:, 1] >= margin_q_v[:, 0] - 1e-7).all().item()
            and (energy_q_v.abs().sum(dim=1) > 1e-8).all().item()
        )
        hard_checks = {
            "all_base_rows_valid": cert["valid_row_count"] == len(classes),
            "sample_counts_positive": cert["min_sample_count"] > 0.0,
            "spectral_coupling_ready": coupling_ready if require_coupling else True,
            "replay_quantiles_ready": replay_stats_ready,
        }
        diagnostic_checks = {
            ...
            "energy_geometry_ok": bool(energy_geometry_preferred_ok or energy_geometry_soft_ok),
            "subspace_warning_gate_ok": cert["max_subspace_overlap"] <= thr["subspace_warn_overlap"],
            "band_guided_geometry_ok": cert["max_band_guided_geometry_conflict"] <= thr["max_guided_geometry_conflict"],
            "spectral_guided_geometry_ok": (
                int(cert.get("spectral_shape_pair_count", 0)) == 0
                or cert["max_spectral_guided_geometry_conflict"] <= thr["max_guided_geometry_conflict"]
            ),
        }
        cert["checks"] = {**hard_checks, **diagnostic_checks}
        cert["hard_checks"] = hard_checks
        cert["diagnostic_checks"] = diagnostic_checks
        cert["soft_state_report"] = True
        cert["valid"] = bool(all(hard_checks.values()) and len(cert["errors"]) == 0)
        if not cert["valid"]:
            cert["errors"].extend([k for k, ok in hard_checks.items() if not ok])
        failed_diag = [k for k, ok in diagnostic_checks.items() if not ok]
        if failed_diag:
            cert["warnings"].append("diagnostic_geometry_risk_soft_only:" + ",".join(failed_diag))
        return cert

    def _print_base_geometry_certificate(self, certificate: Dict[str, Any]) -> None:
        print("[Base Geometry State]")
        if not certificate:
            print("  unavailable")
            return
        ...
        print(
            ...
            f"coupling(min/mean)={float(certificate.get('min_coupling_reliability', 0.0)):.3f}/{float(certificate.get('mean_coupling_reliability', 0.0)):.3f} | "
            f"subspace(max/mean)={float(certificate.get('max_subspace_overlap', 0.0)):.3f}/{float(certificate.get('mean_subspace_overlap', 0.0)):.3f} | "
            f"band(raw max/mean)={float(certificate.get('max_band_similarity', 0.0)):.3f}/{float(certificate.get('mean_band_similarity', 0.0)):.3f} | "
            f"spec(raw max/mean)={float(certificate.get('max_spectral_shape_similarity', 0.0)):.3f}/{float(certificate.get('mean_spectral_shape_similarity', 0.0)):.3f} | "
            ...
            f"guided(band/spec)={float(certificate.get('max_band_guided_geometry_conflict', 0.0)):.3f}/{float(certificate.get('max_spectral_guided_geometry_conflict', 0.0)):.3f}"
        )
    ...
    def _enforce_base_geometry_certificate(self, certificate: Dict[str, Any]) -> None:
        ...
        if bool(certificate.get("valid", False)):
            return
        msg = (
            "Base geometry state contains invalid compact rows. Incremental training requires valid "
            "GeometryBank descriptors. Failed hard checks: "
            f"{certificate.get('errors', [])}"
        )
        # No hard overlap/spectral handoff gate is used. Invalid compact rows are
        # still real code errors and are normally caught earlier by bank validation.
        print(f"[Base Geometry State WARN] {msg}")
    ...
    # ============================================================
    # Clean strict geometry public structure / assertions / handoff
    # ============================================================
    ...
    def _compute_pair_risks(self, cert: Dict[str, Any], center: torch.Tensor, sub: torch.Tensor, conflict: torch.Tensor, band: torch.Tensor, spec: torch.Tensor) -> None:
        """Pair-level SCTGR conflict diagnostics.

        Raw band/spectral similarity is reported as hard-pair context.  A pair is
        unsafe only when physical similarity coincides with unsafe learned
        geometry or the learned geometry itself violates the energy/subspace gate.
        """
        ...
        unsafe_pairs = []
        hard_pairs = []
        conflict_by_class = {int(c): 0.0 for c in classes}

        band_guided = torch.as_tensor(cert.get("band_guided_geometry_conflict_matrix", []), device=self.device, dtype=torch.float32)
        spec_guided = torch.as_tensor(cert.get("spectral_guided_geometry_conflict_matrix", []), device=self.device, dtype=torch.float32)
        if band_guided.dim() != 2 or band_guided.size(0) != n:
            band_guided = torch.zeros((n, n), device=self.device)
        if spec_guided.dim() != 2 or spec_guided.size(0) != n:
            spec_guided = torch.zeros((n, n), device=self.device)
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                bguide_ij = float(max(band_guided[i, j].item(), band_guided[j, i].item())) if band_guided.numel() else 0.0
                sguide_ij = float(max(spec_guided[i, j].item(), spec_guided[j, i].item())) if spec_guided.numel() else 0.0
                dist_ij = float(center[i, j].item()) if center.numel() else 0.0

                if band_ij > float(thr.get("max_band_similarity", 0.90)):
                    context.append("raw_band_hard_pair")
                if spec_ij > float(thr.get("max_spectral_shape_similarity", 0.85)):
                    context.append("raw_spectral_hard_pair")

                if sub_ij > float(thr.get("subspace_warn_overlap", 0.72)):
                    reasons.append("subspace_warning_gate")
                if conf_ij > float(thr.get("max_geometry_conflict", 1.35)):
                    reasons.append("energy_geometry")
                if bguide_ij > float(thr.get("max_guided_geometry_conflict", 0.18)):
                    reasons.append("band_guided_geometry")
                if sguide_ij > float(thr.get("max_guided_geometry_conflict", 0.18)):
                    reasons.append("spectral_guided_geometry")

                pair_score = max(
                    0.0,
                    sub_ij if "subspace_warning_gate" in reasons else 0.0,
                    conf_ij if "energy_geometry" in reasons else 0.0,
                    bguide_ij,
                    sguide_ij,
                )
                ...
                if context:
                    hard_pairs.append({
                        ...
                        "band_guided_geometry_conflict": bguide_ij,
                        "spectral_guided_geometry_conflict": sguide_ij,
                    })
                ...
                if reasons:
                    item = {
                        "class_i": classes[i],
                        "class_j": classes[j],
                        "geometry_conflict_score": float(pair_score),
                        "risk": float(pair_score),  # backward-compatible alias
                        "reasons": reasons,
                        "context": context,
                        "center_distance": dist_ij,
                        "subspace_overlap": sub_ij,
                    ...
                    conflict_by_class[classes[i]] = max(float(conflict_by_class[classes[i]]), float(pair_score))
                    conflict_by_class[classes[j]] = max(float(conflict_by_class[classes[j]]), float(pair_score))

        unsafe_pairs = sorted(unsafe_pairs, key=lambda x: x["geometry_conflict_score"], reverse=True)
        hard_pairs = sorted(hard_pairs, key=lambda x: max(float(x.get("band_similarity", 0.0)), float(x.get("spectral_similarity", 0.0))), reverse=True)
        cert["unsafe_class_pairs"] = unsafe_pairs
        cert["hard_physical_pairs"] = hard_pairs

        geometry_conflict_by_class = {int(k): float(v) for k, v in conflict_by_class.items()}
    ...
    def _add_handoff_recommendations(self, cert: Dict[str, Any]) -> None:
        ...
        for cls in cert.get("class_ids", []):
            cls_conflict = conflict_by_class.get(int(cls), 0.0)
            replay_plan[int(cls)] = int(min(max_replay, max(base_replay, round(base_replay * (1.0 + cls_conflict)))))

        max_pair_conflict = max(conflict_by_class.values()) if conflict_by_class else 0.0
        max_guided = max(
            float(cert.get("max_band_guided_geometry_conflict", 0.0)),
            float(cert.get("max_spectral_guided_geometry_conflict", 0.0)),
        )
        max_conflict = max(float(cert.get("max_geometry_conflict", 0.0)), max_pair_conflict, max_guided)
        ...
        # Main SCTGR path does not use old-row transport.
        cert["recommended_transport_activation"] = False
        cert["transport_required"] = False

        cert["recommended_reserved_alignment"] = bool(
            max_guided > self._base_cfg_float("base_cert_max_guided_geometry_conflict", 0.18)
            or float(cert.get("max_geometry_conflict", 0.0)) > self._base_cfg_float("base_cert_max_geometry_conflict", 1.35)
            or float(cert.get("max_subspace_overlap", 0.0)) > self._base_cfg_float("base_cert_subspace_warn_overlap", 0.72)
        )
    ...
    def finalize_base_handoff(self, phase_class_ids: Iterable[int], certificate: Dict[str, Any], history: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        handoff = {
            ...
            "coupling_reliability": certificate.get("coupling_reliability", {}),
            "energy_quantiles": certificate.get("energy_quantiles", {}),
            "margin_quantiles": certificate.get("margin_quantiles", {}),
            "replay_ready": bool(certificate.get("hard_checks", {}).get("replay_quantiles_ready", False)),
    ...
    def _train_epoch_base_geometry(
        ...
        stat_sums = {
            ...
            "weighted_gics": 0.0,
            "energy_margin": 0.0,
            "energy_margin_weighted": 0.0,
            "energy_margin_violation": 0.0,
            "energy_margin_gap": 0.0,
            "gics_anchors": 0.0,
            "gics_pos": 0.0,
            "pgr": 0.0,
            "pgr_unweighted": 0.0,
            ...
            "pgr_band_guided_conflict_mean": 0.0,
            "pgr_band_guided_conflict_max": 0.0,
            "spectral_shape": 0.0,
            "spectral_shape_raw": 0.0,
            "spectral_shape_mean_similarity": 0.0,
            "spectral_shape_pairs": 0.0,
            "spectral_shape_guided_conflict_mean": 0.0,
            "spectral_shape_guided_conflict_max": 0.0,
            "spectral_active": 0.0,
            "batch_compact": 0.0,
        ...
        for batch in loader:
            ...
            loss_out = unified_spectral_geometry_loss(
                ...
                base_energy_margin_weight=self._base_cfg_float("base_energy_margin_weight", 0.15),
                base_energy_margin=self._base_cfg_float("base_energy_margin", 0.25),
                base_energy_variance_floor=self._base_cfg_float("geom_var_floor", 5e-4),
                pgr_compact_weight=self._base_cfg_float("pgr_compact_weight", 0.15),
                pgr_center_weight=self._base_cfg_float("pgr_center_weight", 0.20),
                pgr_subspace_weight=self._base_cfg_float("pgr_subspace_weight", 0.10),
                pgr_band_weight=self._base_cfg_float("pgr_band_weight", 0.05),
            ...
            margin_out = self._base_batch_geometry_energy_margin(
                features, y_local, num_classes=num_base
            )
            loss_out.update(margin_out)
            self._assert_mandatory_base_loss_parts(
                loss_out,
                spectral_summary_is_physical=bool(spectral_summary_is_physical),
            )
            ce = self._batch_balanced_ce(logits, y_local, num_base)
            # The differentiable SRPGR/GICS/PGR graph is in loss_out["total"]
            # because ce_weight=0.0.  The geometry-energy margin is constructed
            # explicitly above against batch low-rank geometry.
            ...
            energy_margin_total = margin_out["base_energy_margin_weighted"]
            loss = self._base_cfg_float("base_ce_weight", 1.0) * ce + srpgr_total + energy_margin_total
            ...
            stat_sums["energy_margin"] += _lstat("base_energy_margin")
            stat_sums["energy_margin_weighted"] += _lstat("base_energy_margin_weighted", _lstat("base_energy_margin"))
            stat_sums["energy_margin_violation"] += _lstat("base_energy_margin_violation")
            stat_sums["energy_margin_gap"] += _lstat("base_energy_margin_gap")
            ...
            stat_sums["pgr_band_guided_conflict_mean"] += _lstat("base_pgr_band_guided_conflict_mean")
            stat_sums["pgr_band_guided_conflict_max"] += _lstat("base_pgr_band_guided_conflict_max")
            ...
            stat_sums["spectral_shape_guided_conflict_mean"] += _lstat("base_spectral_shape_guided_conflict_mean")
            stat_sums["spectral_shape_guided_conflict_max"] += _lstat("base_spectral_shape_guided_conflict_max")
    ...
    def _select_base_checkpoint_score(self, val_stats: Dict, geom_stats: Optional[Dict] = None) -> float:
        ...
        if metric in {"geometry_score", "geo", "reserve"}:
            ...
            overlap_warn = self._base_cfg_float("base_cert_subspace_warn_overlap", 0.72)
            overlap_excess = max(0.0, overlap_max - overlap_warn)
            guided_band = float(val_stats.get("cert_max_band_guided_geometry_conflict", 0.0))
            guided_spec = float(val_stats.get("cert_max_spectral_guided_geometry_conflict", 0.0))
            guided_conflict = max(guided_band, guided_spec)
            energy_margin_violation = float(val_stats.get("base_energy_margin_violation", 0.0))
            energy_margin_gap = float(val_stats.get("base_energy_margin_gap", 0.0))
            gap_target = max(self._base_cfg_float("base_score_energy_gap_target", 1.0), 1e-6)
            # Raw gaps are unbounded and previously dominated checkpoint
            # selection.  Saturation rewards reaching a healthy gap but not
            # inflating it at the expense of overlap or conflict.
            energy_gap_reward = float(torch.tanh(torch.tensor(max(0.0, energy_margin_gap) / gap_target)).item())
            ...
            g_overlap_excess = self._base_cfg_float("base_score_overlap_excess_gamma", 8.0)
            g_overlap_mean = self._base_cfg_float("base_score_overlap_mean_gamma", 0.5)
            d_guided = self._base_cfg_float("base_score_guided_conflict_delta", 6.0)
            cert_bonus = self._base_cfg_float("base_score_cert_bonus", 3.0)
            e_viol_beta = self._base_cfg_float("base_score_energy_violation_beta", 10.0)
            e_gap_alpha = self._base_cfg_float("base_score_energy_gap_alpha", 2.0)
            return (
                acc
                + a * reserve
                + e_gap_alpha * energy_gap_reward
                - e_viol_beta * max(0.0, energy_margin_violation)
                + cert_bonus * cert_valid
                - b_conf_max * conflict_max
                - b_conf_mean * conflict_mean
                - g_overlap_excess * overlap_excess
                - g_overlap_mean * overlap_mean
                - d_guided * guided_conflict
            )
    ...
    def train_base_phase(self, phase, epochs, batch_size=64, lr=1e-4) -> Dict:
        ...
        try:
            ...
            print("==== Base Phase Training | Spectral-Coupled Tangent Geometry Replay Preparation ====")
            ...
            history = {
                ...
                "base_spectral_shape_pairs": [], "base_spectral_shape_guided_conflict_mean": [],
                "base_spectral_shape_guided_conflict_max": [], "base_spectral_active": [],
                ...
                "base_cert_max_geometry_conflict": [], "base_cert_max_band_guided_geometry_conflict": [],
                "base_cert_max_spectral_guided_geometry_conflict": [], "base_cert_valid": [],
                "base_replay_statistics": [],
                "base_replay_max_violation": [], "base_replay_min_margin_q05": [],
            }

            best_score = -1e18
            # Lexicographic health selection: valid rows, acceptable margin
            # violation, validation accuracy, guided conflict, geometry conflict,
            # overlap, then the saturated scalar score as a final tie-breaker.
            best_key = (-1, -1, -1e18, -1e18, -1e18, -1e18, -1e18)
            best_state = None
            best_epoch_cert: Optional[Dict[str, Any]] = None
            no_improve = 0
            epochs = int(epochs)
            ...
            print("[Base Objective] MANDATORY: balanced CE + GICS + PGR(compact, center, subspace, band, volume) + base geometry-energy margin on canonical projected z-space.")
            ...
            for epoch in range(epochs):
                ...
                epoch_replay_stats = self._update_base_replay_statistics(
                    train_loader, phase_class_ids, allow_frozen_update=False
                )
                ...
                for k in (
                    "ce", "srpgr", "compact_sep", "gics", "weighted_gics",
                    "energy_margin", "energy_margin_weighted", "energy_margin_violation", "energy_margin_gap",
                    "gics_anchors", "gics_pos", "pgr", "pgr_unweighted",
                    ...
                    "pgr_subspace_max_overlap", "pgr_band_max_similarity", "pgr_band_guided_conflict_mean", "pgr_band_guided_conflict_max",
                    "spectral_shape", "spectral_shape_raw", "spectral_shape_mean_similarity", "spectral_shape_pairs",
                    "spectral_shape_guided_conflict_mean", "spectral_shape_guided_conflict_max", "spectral_active",
                    "batch_compact", "batch_center_margin", "batch_min_center_margin",
                ):
                ...
                history["base_cert_max_band_guided_geometry_conflict"].append(float(epoch_cert.get("max_band_guided_geometry_conflict", 0.0)))
                history["base_cert_max_spectral_guided_geometry_conflict"].append(float(epoch_cert.get("max_spectral_guided_geometry_conflict", 0.0)))
                history["base_cert_valid"].append(1.0 if bool(epoch_cert.get("valid", False)) else 0.0)
                history["base_replay_statistics"].append(self._jsonify(epoch_replay_stats))
                replay_max_violation = max(
                    (float(v.get("violation_rate", 0.0)) for v in epoch_replay_stats.values()),
                    default=1.0,
                )
                replay_min_margin_q05 = min(
                    (float(v.get("margin_q05", 0.0)) for v in epoch_replay_stats.values()),
                    default=0.0,
                )
                history["base_replay_max_violation"].append(replay_max_violation)
                history["base_replay_min_margin_q05"].append(replay_min_margin_q05)
                ...
                score_stats["base_energy_margin_violation"] = float(replay_max_violation)
                score_stats["base_energy_margin_gap"] = float(replay_min_margin_q05)
                ...
                print(
                    ...
                    f"EnergyMargin={float(base_stats.get('energy_margin_weighted', 0.0)):.4f} | "
                    f"EViol={float(base_stats.get('energy_margin_violation', 0.0)):.3f} | "
                    f"EGap={float(base_stats.get('energy_margin_gap', 0.0)):.4f} | "
                    f"ReplayViolMax={float(replay_max_violation):.3f} | "
                    f"ReplayQ05Min={float(replay_min_margin_q05):.4f} | "
                    ...
                    f"PGRBandGuide={float(base_stats.get('pgr_band_guided_conflict_max', 0.0)):.4f} | "
                    f"SpecGuide={float(base_stats.get('spectral_shape_guided_conflict_max', 0.0)):.4f} | "
                    ...
                    f"GuidedMax={max(float(epoch_cert.get('max_band_guided_geometry_conflict', 0.0)), float(epoch_cert.get('max_spectral_guided_geometry_conflict', 0.0))):.4f} | "
                    f"Reserve={float(epoch_cert.get('geometry_reserve_score', 0.0)):.4f} | "
                    f"Score={float(score):.4f} | State={'PASS' if bool(epoch_cert.get('valid', False)) else 'CONFLICT'}"
                )

                # Checkpoint selection is geometry-health aware.  Accuracy is
                # optimized only after valid replay rows and an acceptable real
                # margin-violation rate are established.  Raw energy-gap growth
                # is only a saturated scalar tie-breaker.
                cert_priority = 1 if bool(epoch_cert.get("valid", False)) else 0
                violation = float(replay_max_violation)
                violation_ok = 1 if violation <= self._base_cfg_float("base_checkpoint_max_energy_violation", 0.02) else 0
                guided = max(
                    float(epoch_cert.get("max_band_guided_geometry_conflict", 0.0)),
                    float(epoch_cert.get("max_spectral_guided_geometry_conflict", 0.0)),
                )
                candidate_key = (
                    cert_priority,
                    violation_ok,
                    float(val_stats.get("acc", 0.0)),
                    -guided,
                    -float(epoch_cert.get("max_geometry_conflict", 0.0)),
                    -float(epoch_cert.get("max_subspace_overlap", 0.0)),
                    float(score),
                )
                if candidate_key > best_key:
                    ...
                    best_epoch_cert = dict(epoch_cert)
                    no_improve = 0
                else:
                    no_improve += 1
            ...
            if best_state is not None:
                ...
                self._set_model_phase_and_old_count(phase, 0)
                if isinstance(best_epoch_cert, dict):
                    self._best_base_epoch_certificate = best_epoch_cert
                    print(
                        "[Base] Restored state-aware best state | "
                        f"State={'PASS' if bool(best_epoch_cert.get('valid', False)) else 'CONFLICT'} | "
                        f"Score={best_score:.4f}"
                    )

            print("[Base] Final replay-ready GeometryBank rebuild from restored best canonical z-space.")
            # Do not call the generic finalizer here: it rebuilds and freezes in
            # one operation, which would leave class-local zero energy/margin
            # quantiles.  Build first, measure the complete rival field, then
            # freeze the finished replay rows.
            self.rebuild_base_geometry_bank(phase, phase_class_ids, split="train")
            final_replay_stats = self._update_base_replay_statistics(
                train_loader, phase_class_ids, allow_frozen_update=False
            )
            history["final_base_replay_statistics"] = self._jsonify(final_replay_stats)
            if hasattr(self.dataset, "finalize_phase"):
                self.dataset.finalize_phase(phase)
            # Remain in phase-0 mode until the handoff assertion promotes the
            # committed base rows to the old set for phase 1.
            self.model.current_phase = 0
            self._assert_base_bank_valid(phase_class_ids)

            # Freeze only after feature geometry, spectral tangent coupling and
            # aggregate replay quantiles are complete.
            if hasattr(self.model, "assert_base_handoff_ready"):
                ...
                if hasattr(gb, "assert_phase0_base_handoff_ready"):
                    ...
                    gb.freeze_classes_up_to(max(phase_class_ids) + 1)
            self._assert_base_bank_valid(phase_class_ids)
            ...
            run_persistent_diagnostics = self._base_cfg_bool("save_geometry_diagnostics", True) or self._base_cfg_bool("print_base_geometry_diagnostics", True)
            if run_persistent_diagnostics and hasattr(self, "diagnose_full_base_geometry"):
                try:
...
#         keeps the command-line/incremental configuration intact, while ensuring
#         that the base phase trains only backbone/projection + temporary CE head
#         and builds a clean Low-Rank Geometry Replay + Residual Geometry Adaptation / GeometryBank handoff.
#         """
#         # Keep the method coherent: GICS/PGR/SpectralShape are internal parts of
#         # SRPGR. Do not mutate shared args here. Legacy spectral-GICS knobs are
#         # ignored by the cleaned base objective and must be controlled by the
...
#         This is a safe no-op unless physical raw spectra are available and the
#         Low-Rank Geometry Replay + Residual Geometry Adaptation loss file exposes spectral_shape_discrimination_loss.
#         """
#         if spectral_shape_discrimination_loss is None:
#             z = self._zero(features)
#             return {"total": z, "spectral_shape": z, "mean_similarity": z, "pair_count": z, "valid_class_count": z}
...
#         head_train_acc: Optional[float] = None,
#     ) -> Dict[str, Any]:
#         """Create the base-to-incremental Low-Rank Geometry Replay + Residual Geometry Adaptation geometry certificate.
...
#     def _compute_pair_risks(self, cert: Dict[str, Any], center: torch.Tensor, sub: torch.Tensor, conflict: torch.Tensor, band: torch.Tensor, spec: torch.Tensor) -> None:
#         """Pair-level Low-Rank Geometry Replay + Residual Geometry Adaptation conflict diagnostics.
...
#         # Main Low-Rank Geometry Replay + Residual Geometry Adaptation path does not use old-row transport.
#         cert["recommended_transport_activation"] = False
#         cert["transport_required"] = False
...
#                 acc
#                 + a * reserve
#                 + e_gap_alpha * max(0.0, energy_margin_gap)
#                 - e_viol_beta * max(0.0, energy_margin_violation)
#                 + cert_bonus * cert_valid
#                 - b_conf_max * conflict_max
#                 - b_conf_mean * conflict_mean
...
#             print("==== Base Phase Training | Low-Rank Geometry Replay + Residual Geometry Adaptation Preparation ====")
#             self.dataset.start_phase(phase)
#             phase_class_ids = [int(c) for c in self.dataset.phase_to_classes[phase]]
#             if not phase_class_ids:
#                 raise RuntimeError("Phase 0 has no base classes.")
...
#             best_score = -1e18
#             best_key = (-1, -1e18)  # (certificate_valid_priority, checkpoint_score)
#             best_state = None
#             best_epoch_cert: Optional[Dict[str, Any]] = None
#             no_improve = 0
#             epochs = int(epochs)
...
#                 score_stats.update({f"cert_{k}": v for k, v in epoch_cert.items() if isinstance(v, (int, float, bool))})
#                 score_stats["geometry_score"] = float(geom_stats.get("geometry_reserve_score", 0.0))
#                 score_stats["base_energy_margin_violation"] = float(base_stats.get("energy_margin_violation", 0.0))
#                 score_stats["base_energy_margin_gap"] = float(base_stats.get("energy_margin_gap", 0.0))
#                 score = self.compute_checkpoint_score(score_stats, geom_stats)
#                 history["checkpoint_score"].append(float(score))
...
#                 # Checkpoint selection must be state-aware.  A tiny
#                 # score increase from a CONFLICT epoch must not replace an
#                 # earlier PASS epoch, because the final state is consumed by
#                 # the incremental phase.
#                 cert_priority = 1 if bool(epoch_cert.get("valid", False)) else 0
#                 candidate_key = (cert_priority, float(score))
#                 if candidate_key > best_key:
#                     best_key = candidate_key
#                     best_score = float(score)
#                     best_state = self._capture_state()
